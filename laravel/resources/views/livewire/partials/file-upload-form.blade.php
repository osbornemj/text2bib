<div>
    ...
    <div class="space-y-6 my-4">
        <div class="sm:pt-0">
            ...
            <p>
                <span class="bg-yellow-200 dark:bg-yellow-700 mr-2 text-sm">NEW</span> If you wish, you can ask the system (by selecting an option in the  form below) to retrieve from <x-link href="https://crossref.org" target="_blank">Crossref</x-link> each item in your file that has a valid <code>doi</code>.  You will then be able to choose either the components of the reference retrieved from Crossref or the ones extracted from your file by <code>text2bib</code>.  This feature is currently experimental.  You are limited to <b>{{ config('constants.crossref_quota') }}</b> queries to Crossref each day (00:00 to 24:00 UTC).  You have <b>{{ $crossrefQuotaRemaining }}</b> {{ Str::plural('query', $crossrefQuotaRemaining) }} remaining today.
            </p>
        </div>
    </div>
    ...
    <div>
        <x-input-label for="use_crossref" :value="__('Retrieve matching reference from Crossref.org?')" class="mt-4 mb-1" />
