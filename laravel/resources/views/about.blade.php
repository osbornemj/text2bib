<x-app-layout>
    ...
    <div class="mt-6 px-4 sm:px-4 lg:px-4">
        ...
        <x-paragraph class="mt-4">
            The converter consists of a large number of hand-coded rules for extracting the author, title, and publication information from character strings that represent references.  I make improvements to it by occasionally looking for errors in the conversions for files uploaded by users.  When I see an error, I add the source and the correct version of the BibTeX entry to a database table of examples and modify the code to deal with it correctly, while still correctly converting all the other examples.  The examples table contains over 440 items.  (Unfortunately error report by users are few and far between, and almost no user responds to clarifactory questions.)
        </x-paragraph>
        <x-paragraph class="mt-4">
